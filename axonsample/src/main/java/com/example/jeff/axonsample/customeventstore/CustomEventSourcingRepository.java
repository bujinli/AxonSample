...
public class CustomEventSourcingRepository<T> extends EventSourcingRepository<T> {
  ...
  public LockAwareAggregate<T, EventSourcedAggregate<T>> load(String aggregateIdentifier,
    ...
    LockAwareAggregate<T, EventSourcedAggregate<T>> aggregate =
        (LockAwareAggregate<T, EventSourcedAggregate<T>>) ((Map) aggregates)
  ...
  public static class Builder<T> extends EventSourcingRepository.Builder<T> {
    ...
    @Override
    public CustomEventSourcingRepository.Builder<T> handlerDefinition(
    ...
    @Override
    public CustomEventSourcingRepository.Builder<T> aggregateModel(
    ...
    /**
     * {@inheritDoc}
     * If this Builder is used to instantiate a {@link CachingEventSourcingRepository}, do note
     * that an optimistic
     ...
     *
     * @param eventStore an {@link EventStore} that holds the event com.craftsman.eventsourcing
     *                   .stream this repository needs to event source
     *                   an Aggregate
     * @return the current Builder instance, for fluent interfacing
     */
    public CustomEventSourcingRepository.Builder<T> eventStore(
     ...
     * @return the current Builder instance, for fluent interfacing
     */
    public CustomEventSourcingRepository.Builder<T> aggregateFactory(
     ...
     *
     * @param repositoryProvider a {@link RepositoryProvider} servicing repositories for specific
     *                          aggregate types
     * @return the current Builder instance, for fluent interfacing
     */
    public CustomEventSourcingRepository.Builder<T> repositoryProvider(
    ...
    /**
     * Initializes a {@link EventSourcingRepository} or {@link CachingEventSourcingRepository} as
     * specified through
     * this Builder. Will return a CachingEventSourcingRepository if {@link #cache(Cache)} has
     * been set. Otherwise
     ...
     * {@link EventSourcingRepository} will instantiate based on an event com.craftsman
    ...
    protected void validate() {
      ...
      if (aggregateFactory == null) {
        assertNonNull(aggregateType,
            "No AggregateFactory is set, whilst either it or the aggregateType is a hard "
                + "requirement");
        return;
      }
